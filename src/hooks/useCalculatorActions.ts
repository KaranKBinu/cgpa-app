...
export function useCalculatorActions({
  ...
  const downloadAsPDF = async (semIdOrEvent?: string | any) => {
    ...
    doc.setFontSize(22); doc.setTextColor(16, 185, 129); doc.text(isSingleSem ? "Semester Performance Report" : "PolyCGPA Calculator Cumulative Report", 14, 22);
    ...
    doc.save(isSingleSem ? `Transcript_${targets[0].name.replace(/\s+/g, '_')}_${program.code}.pdf` : `PolyCGPA_Calculator_FullReport_${program.code}.pdf`);
  };
