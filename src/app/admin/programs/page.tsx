    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {programs.map((program) => (
          <div key={program.id} className="p-8 rounded-[2.5rem] bg-card/50 border border-border/50 hover:border-emerald-500/30 transition-all group">
            <div className="flex items-start justify-between mb-8">
              <div className="space-y-2">
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em]">{program.code}</span>
                <h3 className="text-2xl font-black text-foreground tracking-tight leading-tight">{program.name}</h3>
                <p className="text-muted-foreground text-[10px] font-black uppercase tracking-widest">{program.scheme}</p>
              </div>
              <div className="h-12 w-12 rounded-2xl bg-card/80 flex items-center justify-center text-muted-foreground group-hover:text-emerald-500 group-hover:bg-emerald-500/10 transition-all">
            ...
            <div className="pt-8 border-t border-border/50 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Link href={`/admin/programs/${program.id}`} className="p-3 rounded-xl bg-card/80 hover:bg-card transition-all text-muted-foreground hover:text-foreground">
      ...
      {programs.length === 0 && (
        <div className="py-20 text-center bg-card/50 border border-dashed border-border/50 rounded-[3rem]">
           ...
           <p className="text-muted-foreground font-bold">No academic programs registered yet.</p>
        </div>
      )}
    </div>
  );
