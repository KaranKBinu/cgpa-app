    ...
    <html lang="en" className="dark">
      <body className={`${inter.variable} ${outfit.variable} font-inter min-h-screen bg-[#050505] text-white selection:bg-emerald-500/30 overflow-x-hidden`}>
        <SessionProvider>
          ...
          <header className="absolute top-0 left-0 right-0 z-[60] p-4 lg:p-6 text-white">
            <nav className="mx-auto max-w-7xl px-4 lg:px-8 h-16 lg:h-20 rounded-[1.5rem] lg:rounded-[2rem] border border-white/10 bg-black/40 backdrop-blur-2xl shadow-2xl flex items-center justify-between">
              <Link href="/" className="flex items-center gap-3 group transition-all active:scale-95">
                ...
                <div className="flex flex-col">
                  <span className="text-base lg:text-lg font-black tracking-tight text-white leading-none">{config.appName}</span>
                  <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">{config.revision}</span>
                </div>
              </Link>
              
              <div className="flex items-center gap-1 lg:gap-3 font-bold text-xs lg:text-sm text-white/60">
                ...
                {user ? (
                  <div className="flex items-center gap-2 lg:gap-4">
                    <div className="hidden lg:flex flex-col items-end mr-2">
                      <span className="text-white text-[11px] leading-tight font-black uppercase tracking-tighter">{user.name}</span>
                  ...
                  <div className="flex items-center gap-2">
                    <Link href="/auth/login" className="px-4 py-2 rounded-xl hover:text-white transition-all font-black uppercase tracking-widest text-[10px]">
                      Login
                    </Link>
